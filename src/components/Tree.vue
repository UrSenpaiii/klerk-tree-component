...
<template>
  <li :class="['p-1', 'border-l',
              {'border-b': hasChildren}, 'border-green-400']"
  >
    <div :class="['px-3', 'py-3', 'flex justify-between items-center gap-x-3', 'cursor-pointer', 'transition', 'duration-300',
                 'hover:bg-green-800', {'bg-green-900': active}, {'hover:bg-neutral-800': !hasChildren},
                 {'border-l-4': active}, 'border-green-500']"
         @click.stop="toggle"
    >
      <div>
        <a :href="`https://www.klerk.ru${node.url}`" target="_blank"
    ...
    <Tree v-show="active" v-for="child in node.children" :key="child.id"
          :node="child"
          :isSelected="selected"
          @select="bubble"
    />
